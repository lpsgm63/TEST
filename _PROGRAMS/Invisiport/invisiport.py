...
def blacklist(ip):
	if ip in whitelist or check_blacklist(ip):
		...
		query = "iptables -A INPUT -s %s -p tcp -m multiport ! --destination-port %s,%s -j DROP" % (ip, PORTSPOOF, COWRIE)
		os.system(query)
		for port in ports:
			query = "iptables -t nat -A PREROUTING -s %s -p tcp --dport %s -j REDIRECT --to-port %s" % (ip,port, PORTSPOOF)
			os.system(query)
		add_blacklist(ip)
	return True
